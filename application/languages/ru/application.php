<?php
/**
 * Created by JetBrains PhpStorm.
 * User: raccoon
 * Date: 01.09.11
 * Time: 17:33
 * To change this template use File | Settings | File Templates.
 */
 return array(
     'Hello'=>'Привет',
     'Good bye'=>'До свидания',

     //User management
     'Users list'=>'Список пользователей',

     //News managment
     'News list'=>'Список новостей',
     'Anons'=>'Анонс',
     'Text'=>'Текст',

     //User
     'Nickname'=>'Имя входа',
     'Full name'=>'Полное имя',
     'Edit user'=>'Редактирование пользователя',

     //Page
     'Page list'=>'Список страниц',
     'Title'=>'Заголовок',
     'Edit page'=>'Редактирование страницы',
     'auto generate url'=>'Генерировать URL автоматически (по заголовку)',
     'Meta tags'=>'Мета теги (ключи для поисковых систем)',

     //Tags
     'Tags'=>'Теги',

     //Info
     'Add info'=>'Добавить информацию',
     'Key'=>'Ключ',
     'Value'=>'Значение',
     'Edit info'=>'Редактировать информацию',

     //Action
     'Edit'=>'Редактировать',
     'Delete'=>'Удалить',
     'Add new item'=>'Добавить позицию',
     'Add'=>'Добавить',

     //Mail
     'Hello, %s!'=>'Здравствуйте, %s!',
     'On site %s has been created account for you.'=>'На сайте %s был создан аккаунт для вас.',
     'Your login'=>'Имя для входа',
     'Your password'=>'Ваш пароль',
     'An «%s» account has been created for you!'=>'Для вас был создан аккаунт на сайте «%s»!',

     //Messages
     'Are you sure you want to remove this item?'=>'Вы действительно хотите удалить эту позицию?',
     
     //errors
     'The list is empty'=>'Список пуст',
     'User with this login have already isset!'=>'Пользователь с таким логином уже существует!',
     'User with this email have already isset!'=>'Пользователь с таким e-mail уже существует!',
     "Value is required and can't be empty"=>"Это обязательное поле, не может быть пустым!"
 );